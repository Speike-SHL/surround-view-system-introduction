...
yaml文件内内容分别为: <br>
1. `camera_matrix`: 相机内参矩阵 <br>
2. `dist_coeffs`: 畸变系数 <br>
3. `resolution`: 分辨率 <br>
4. `project_matrix`: 相机投影矩阵 <br>
5. `scale_xy` 
6. `shift_xy`
...
这一步是通过**联合标定**实现的，即在车的四周地面上摆放标定物，拍摄图像，手动选取对应点，然后获取投影矩阵。
...
+ `shiftWidth`, `shiftHeight`：这两个参数决定了在鸟瞰图中向标定板的外侧看多远。这两个值越大，鸟瞰图看的范围就越大，相应地远处的物体被投影后的**形变**也越严重，所以应酌情选择。
